export default async function handler(req, res) {
  ...
  try {
    ...
    if (response.status === 403) return res.status(403).json({ error: "Competition requires paid plan" });
    if (response.status === 404) return res.status(404).json({ error: "Competition not found or season not available" });
