...
function methodStyle(type) {
  if (type === "finish") return { background: "#E24B4A18", color: "#E24B4A", border: "1px solid #E24B4A30" };
  return { background: "#ffffff08", color: "#888", border: "1px solid #ffffff15" };
}

const grouped = RECENT_FIGHTS.reduce((acc, f) => {
    ...
    <div>
      ...
      <div className={styles.grid}>
        <div className={styles.panel}>
          ...
          {Object.entries(grouped).map(([event, fights]) => (
        ...
        <div>
          ...
          <div className={styles.panel}>
            ...
            <div className={styles.finishBars}>
              {[
                { label: "KO / TKO", count: RECENT_FIGHTS.filter(f => f.method === "KO").length, color: "#E24B4A", max: RECENT_FIGHTS.length },
                { label: "Submission", count: RECENT_FIGHTS.filter(f => f.method === "Sub").length, color: "#378ADD", max: RECENT_FIGHTS.length },
                { label: "Decision", count: decisions, color: "#888780", max: RECENT_FIGHTS.length },
              ].map((b) => (
                <div key={b.label} className={styles.barRow}>
                  ...
                  <div className={styles.barTrack}>
                    <div
                      ...
                      style={{ width: `${Math.round((b.count / b.max) * 100)}%`, background: b.color }}
                    />
                  </div>
